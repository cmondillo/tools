...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    # HTTP payment gate below only covers "GET /preview" (see
    # build_routes_config), so it doesn't interfere with /mcp.
    #
    # Unlike the HTTP /preview route (which can lazily sync on its first
    ...
    # next restart/redeploy) and let /preview keep working as it always has.
    # Same "don't crash on a facilitator hiccup" contract as the try/except
    # around payment_gate below, just at startup instead of per-request.
    mcp_app = None
    try:
